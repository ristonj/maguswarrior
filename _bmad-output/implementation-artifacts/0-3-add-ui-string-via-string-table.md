...
- [x] [Review][Patch] Missing `ui_strings.en.translation` + `ui_strings.csv.import` not committed [data/strings/] — Resolved: `ui_strings.csv.import` committed; `ui_strings.en.translation` is intentionally gitignored (`*.translation` in `.gitignore`) and regenerated by the Godot editor on project open from the committed `.csv.import` metadata. `project.godot` references the correct path. `locale/fallback="en"` and `window/handheld/orientation=0` restored after editor rewrote the file.
